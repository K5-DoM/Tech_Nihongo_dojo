...
- 初回質問は getFirstQuestion で「研究概要を3分で説明してもらう」旨を明示して生成

## 7.2.2 プロフィール文脈の注入（面接官が把握している前提）
- 面接開始時点のプロフィールを `interviews.profile_snapshot` に保存し、**面接中は固定**して system prompt に注入する
  - 面接中にプロフィールを編集しても、当該セッションの文脈は変わらない
- 注入形式は「箇条書き短文 + 未設定は（未設定）」とし、LLMに **推測補完しない** ことを明示する
- 目的:
  - 面接官が候補者背景（専攻・研究テーマ・志望職種など）を理解した上で、質問や深掘りを自然に行う
  - セッションの再現性（後から見返しても同じ条件で進行）を担保する
## 7.3 JSONスキーマ
```json
{
  "type": "object",
...
- **目標職種**: `interviews.profile_snapshot.targetRole`（未設定の場合は「（未設定）」として渡す）
